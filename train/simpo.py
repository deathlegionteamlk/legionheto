...
class SimPOTrainer(BaseTrainer):
    ...
    def compute_loss(self, model, inputs):
        ...
        chosen_lengths = (chosen_input_ids != -100).sum(dim=-1).float()
        rejected_lengths = (rejected_input_ids != -100).sum(dim=-1).float()

        chosen_rewards = policy_chosen_logps / chosen_lengths.clamp(min=1)
        rejected_rewards = policy_rejected_logps / rejected_lengths.clamp(min=1)
